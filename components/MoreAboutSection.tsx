...
const MoreAboutSection = (props: Props) => {
  ...
  return (
    <div className="text-baBody dark:text-baLight pb-24 px-20">
      <h1 className="text-baDark dark:text-baSubtle text-headerTwo font-ba_large pb-5">
        ...
        {moreAboutData.map((data: MoreAboutDataInterface, index: number) => (
          <div key={index} className='flex w-[500px] items-center gap-4 cursor-pointer' onMouseOver={() => handleMouseOver(index)} onMouseLeave={() => handleMouseLeave(index)}>
            <div className='transition transform hover:scale-105 duration-700'>
              <h3 className={`text-baDark dark:text-baLight ${hoverStates[index] ? "text-[33px]" : "text-headerThree"} font-ba_medium`}>{data?.header}</h3>
              <span className="text-headerSix">{data?.text}</span>
            </div>
            <div className={`p-2 border-baPrimary-400 border-[0.5px] w-fit h-fit rounded-full cursor-pointer  transition-all duration-300 hover:bg-baPrimary hover:text-baLight ${hoverStates[index] ? "bg-baPrimary text-baLight" : "bg-none text-baBody dark:text-baLight"}`}>
